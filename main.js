...
var UPIMPlugin = class extends obsidian.Plugin {
	async onload() {
	...
	async uploadImage() {
		...
		if (!this.settings.apiUrl || !this.settings.token) {
			new obsidian.Notice('请先在设置中配置API地址和Token');
			return;
		}
	...
	async processFiles(files) {
		...
		for (const file of files) {
			try {
				new obsidian.Notice(`正在上传: ${file.name}`);
				...
				new obsidian.Notice(`上传失败 ${file.name}: ${error.message}`);
				failCount++;
			}
		}

		if (successCount > 0) {
			new obsidian.Notice(`成功上传 ${successCount} 个文件${failCount > 0 ? `，失败 ${failCount} 个` : ''}`);
		} else if (failCount > 0) {
			new obsidian.Notice(`所有文件上传失败`);
		}
	}

	async uploadToEasyImage(file) {
		...
		try {
			...
			if (data.result === 'success' && data.code === 200 && data.url) {
				...
				throw new Error(data.msg || '上传失败：未知错误');
			}
		} catch (error) {
	...
	async insertImageLink(imageUrl) {
		const activeView = this.app.workspace.getActiveViewOfType(obsidian.MarkdownView);
		if (!activeView) {
			new obsidian.Notice('请先打开一个markdown文件');
			return;
		}
	...
	async testConnection() {
		try {
			...
			if (response.ok) {
				new obsidian.Notice('API地址连接成功，请确保Token正确');
				return true;
			} else {
				new obsidian.Notice(`API连接测试失败: HTTP ${response.status}`);
				return false;
			}
		} catch (error) {
			new obsidian.Notice(`API连接测试失败: ${error.message}`);
			return false;
		}
	}
};

var UPIMSettingTab = class extends obsidian.PluginSettingTab {
